# 🔐 Keycloak Setup Guide for whrkflow

This guide walks you through setting up Keycloak for authentication and role-based access control in the whrkflow HR platform.
...
# You should see:
# - whrkflow-postgres (port 5432)
# - whrkflow-keycloak-db (internal)
# - whrkflow-keycloak (port 8090)
# - whrkflow-pgadmin (port 5050)
```
...
A realm manages a set of users, credentials, roles, and groups. We'll create a `whrkflow` realm.
...
2. Configure:
   - **Display name**: `whrkflow HR Platform`
...
1. In the `whrkflow` realm, go to "Clients"
2. Click "Create client"
3. **General Settings**:
   - **Client type**: OpenID Connect
   - **Client ID**: `whrkflow-api`
   - Click "Next"
...
3. Fill in:
   ...
   - **Email**: `alice.admin@whrkflow.com`
   - **First name**: `Alice`
   - **Last name**: `Admin`
   - **Email verified**: ON
   - **Enabled**: ON
...
- **Email**: `bob.hrmanager@whrkflow.com`
...
- **Email**: `charlie.manager@whrkflow.com`
...
- **Email**: `diana.employee@whrkflow.com`
...
curl -X POST http://localhost:8090/realms/whrkflow/protocol/openid-connect/token \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "client_id=whrkflow-api" \
...
```
Using issuer-uri: http://localhost:8090/realms/whrkflow
```
...
```bash
# Get Alice's token
TOKEN=$(curl -s -X POST http://localhost:8090/realms/whrkflow/protocol/openid-connect/token \
  -d "client_id=whrkflow-api" \
  -d "client_secret=YOUR_CLIENT_SECRET" \
  -d "grant_type=password" \
  -d "username=alice.admin" \
...
2. Verify realm "whrkflow" exists
